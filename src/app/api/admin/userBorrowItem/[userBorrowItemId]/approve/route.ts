...
import { DEFAULT_APPROVED_BORROW_ITEM_MESSAGE } from '@/utils/constants'
...
export async function POST(
  ...
  try {
    ...
    await prisma.item.update({
      ...
      data: {
        stock: {
          decrement: foundUserBorrowItem.amount,
        },
      },
    })

    const approvedUserBorrowItem = await prisma.userBorrowItem.update({
      where: {
        id: userBorrowItemId,
      },
      data: {
        status: {
          connectOrCreate: {
            create: {
              ...
              message: body.message ?? DEFAULT_APPROVED_BORROW_ITEM_MESSAGE,
            },
            where: {
    ...
    return NextResponse.json(approvedUserBorrowItem, {
      status: 201,
    })
  } catch (error) {
